...
impl View {
    pub fn new(resources: &mut Resources, visible: Rect, state: &GroundState)
        ...
        let mut view = View {
            ...
            platforms: vec![
                Platform::new(resources, state, 0),
                Platform::new(resources, state, 1),
                Platform::new(resources, state, 2),
                Platform::new(resources, state, 3),
                Platform::new(resources, state, 4),
                Platform::new(resources, state, 5),
                Platform::new(resources, state, 6),
            ],
            arrows: vec![
                ArrowPair::new(resources, 0, 8),
                ArrowPair::new(resources, 1, 5),
                ArrowPair::new(resources, 2, 6),
                ArrowPair::new(resources, 3, 7),
                ArrowPair::new(resources, 4, 5),
                ArrowPair::new(resources, 5, 3),
                ArrowPair::new(resources, 6, 4),
            ],
    ...
    fn floor_pt_for_pos(&self, pos: i32) -> Point {
        Point::new(Platform::pos_to_left(pos) + 16, 320)
    }
...
impl Element<Game, PuzzleCmd> for View {
    fn draw(&self, game: &Game, canvas: &mut Canvas) {
        ...
        if self.platforms_and_arrows_visible {
            ...
            self.platforms.draw(state, canvas);
        }
    ...
    fn handle_event(&mut self, event: &Event, game: &mut Game)
        ...
        if !action.should_stop() && self.platforms_and_arrows_visible {
            let subaction = self.platforms.handle_event(event, state);
...
impl Platform {
    fn new(resources: &mut Resources, state: &GroundState, platform_row: i32)
           -> Platform {
        let left = Platform::pos_to_left(state.get_position(platform_row));
        Platform {
    ...
    fn top(&self) -> i32 { 88 + 32 * self.row }

    fn top_point_for_pos(&self, pos: i32) -> Point {
        Point::new(Platform::pos_to_left(pos) + 16, self.top())
    }

    fn set_position(&mut self, pos: i32) {
        self.left = Platform::pos_to_left(pos);
        self.goal = self.left;
    }
    ...
    fn travel_time(from_pos: i32, to_pos: i32) -> f64 {
        let dist = (Platform::pos_to_left(from_pos) -
                    Platform::pos_to_left(to_pos))
                       .abs();
...
impl Element<GroundState, ()> for Platform {
    fn draw(&self, _: &GroundState, canvas: &mut Canvas) {
        let index = ((self.left % 24) / 12) as usize;
        canvas.draw_sprite(&self.sprites[index],
                           Point::new(self.left, self.top()));
    }

    fn handle_event(&mut self, event: &Event, _state: &mut GroundState)
                    -> Action<()> {
        match event {
            &Event::ClockTick => {
                if self.goal != self.left {
                    self.left = if self.goal < self.left {
                        cmp::max(self.goal, self.left - PLATFORM_SPEED)
                    } else {
                        cmp::min(self.goal, self.left + PLATFORM_SPEED)
                    };
                    Action::redraw()
                } else {
                    Action::ignore()
...
impl ArrowPair {
    fn new(resources: &mut Resources, row: i32, delta: i32) -> ArrowPair {
        ArrowPair {
    ...
    fn rect(&self) -> Rect { Rect::new(480, 96 + 32 * self.row, 48, 16) }
}

impl Element<GroundState, (i32, i32)> for ArrowPair {
    fn draw(&self, _: &GroundState, canvas: &mut Canvas) {
    ...
    fn handle_event(&mut self, event: &Event, state: &mut GroundState)
                    -> Action<(i32, i32)> {
        match event {
            &Event::ClockTick => {
            ...
            &Event::MouseDown(pt) if !state.is_solved() => {
                ...
                if rect.contains(pt) {
                    if pt.x() - rect.x() <= (rect.width() / 2) as i32 {
                        self.blink_left = BLINK_FRAMES;
                        Action::redraw().and_return((self.row, -self.delta))
                    } else {
                        self.blink_right = BLINK_FRAMES;
                        Action::redraw().and_return((self.row, self.delta))
                    }
                } else {
                    Action::ignore()
