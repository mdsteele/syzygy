...
use elements::{Hud, HudCmd, HudInput, PuzzleCmd, PuzzleView, Scene,
               ScreenFade, Theater};
use gui::{Action, Canvas, Element, Event, Point, Rect, Resources};
...
pub struct View {
    ...
    hud: Hud,
    undo_stack: Vec<(Point, Direction)>,
    redo_stack: Vec<(Point, Direction)>,
}
...
impl View {
    ...
    fn undo(&mut self, _state: &mut WreckedState) {
        if let Some((pos, dir)) = self.undo_stack.pop() {
            self.redo_stack.push((pos, dir));
            // TODO undo
        }
    }

    fn redo(&mut self, _state: &mut WreckedState) {
        if let Some((pos, dir)) = self.redo_stack.pop() {
            self.undo_stack.push((pos, dir));
            // TODO redo
        }
    }

    fn drain_queue(&mut self) {
        for _ in self.theater.drain_queue() {
            // TODO drain queue
        }
    }
}
